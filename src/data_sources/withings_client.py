...
class WithingsClient:
    ...
    def _make_request(self, endpoint: str, params: Dict[str, Any] = None, method: str = 'POST') -> Dict[str, Any]:
        ...
        self.logger.debug(f"[DEBUG] WithingsClient: token_type={self.token_type}, access_token={token_preview}")
        ...
        try:
            ...
            if hasattr(e, 'response') and hasattr(e.response, 'status_code') and e.response.status_code == 401:
                self.logger.debug(f"Received 401 Unauthorized, attempting token refresh")
    ...
    def get_weight_data(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        self.logger.debug(f"Requesting weight data from {start_date.date()} to {end_date.date()} with params: {params}")
        ...
        try:
            ...
            if 'measuregrps' in result:
                ...
                self.logger.debug(f"Successfully fetched {measurement_count} weight measurements from Withings API")
                
                # In debug mode, print the full response
                import json
                self.logger.debug(f"===== WITHINGS API RESPONSE =====\n{json.dumps(result, indent=2)}\n===== END WITHINGS API RESPONSE =====")
            return result
    ...
    def authenticate(self) -> bool:
        ...
        print(f"[Withings Auth] Please visit this URL to authorize the application: {auth_url}")
    ...
    def get_token(self, code: str, state: str) -> None:
        ...
        try:
            ...
            response = requests.post('https://wbsapi.withings.net/v2/oauth2', data=params)
            response.raise_for_status()
            data = response.json()
