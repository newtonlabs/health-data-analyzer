...
class RecoveryChartGenerator(ChartGenerator):
    ...
    def __init__(self, charts_dir: Optional[str] = None):
        ...
        # Define colors for recovery bars with thresholds
        self.green_color = '#2ecc71'   # Green for high recovery (≥70)
        self.yellow_color = '#f1c40f'  # Yellow for moderate recovery (50-69)
        self.red_color = '#e74c3c'     # Red for low recovery (<50)
        self.recovery_threshold_high = 70
        self.recovery_threshold_low = 50
    def generate(self, df: pd.DataFrame, filename: str = "recovery_chart.png") -> str:
        ...
        for score in df['recovery']:
            ...
            elif score >= self.recovery_threshold_low:
        ...
        bars = ax1.bar(x_numeric, df['recovery'], width=0.6, 
                      color=bar_colors, alpha=0.7, zorder=3)
        ...
        recovery_legend = [
            plt.Rectangle((0,0), 1, 1, color=self.green_color, alpha=0.7, label=f"High Recovery (≥{self.recovery_threshold_high})"),
            plt.Rectangle((0,0), 1, 1, color=self.yellow_color, alpha=0.7, label=f"Moderate Recovery ({self.recovery_threshold_low}-{self.recovery_threshold_high-1})"),
            plt.Rectangle((0,0), 1, 1, color=self.red_color, alpha=0.7, label=f"Low Recovery (<{self.recovery_threshold_low})")
        ]
        ...
        if has_sleep_data:
            ...
            sleep_actual_line, = ax2.plot(x_numeric, df['sleep_actual'], 
                                       color="#7B1F1F", linestyle='--', linewidth=1.5, 
            ...
            sleep_need_line, = ax2.plot(x_numeric, df['sleep_need'], 
                                     color="#1E88E5", linestyle='-', linewidth=1.5, 
            ...
            ax2.tick_params(axis='y', colors='#666666', labelsize=8)
            ax2.set_ylabel('Hours', color='#666666', fontsize=9)
        ...
        ax1.set_ylabel('Recovery', color='#666666', fontsize=9)
        ax1.set_xlabel('', color='#666666', fontsize=9)
        ax1.tick_params(axis='x', colors='#666666', labelsize=8)
        ax1.tick_params(axis='y', colors='#666666', labelsize=8)
        
        # Enhanced grid styling - both horizontal and vertical with increased visibility
        ax1.grid(True, linestyle=':', linewidth=0.7, alpha=0.5, color='#cccccc')
...
class NutritionChartGenerator(ChartGenerator):
    ...
    def __init__(self, charts_dir: Optional[str] = None, target_strength: int = 2400, target_rest: int = 2000):
        """
    ...
    def generate(self, df: pd.DataFrame, filename: str = "nutrition_chart.png") -> str:
        ...
        # Colors
        strength_color = "#7B1F1F"  # dark red
        rest_color = "#999999"      # medium gray
    ...
    def generate_stacked(self, df: pd.DataFrame, filename: str = "stacked_nutrition.png") -> str:
        ...
        protein_color = "#7B1F1F"  # Dark red (for protein, most important macro)
        carbs_color = "#333333"    # Black (for carbs)
        fat_color = "#999999"      # Gray (for fat)
        
        # Bar colors - bright blue for strength days, gray for rest days
        strength_day_color = "#1E88E5"  # Bright blue for strength days
        ...
        ax.grid(True, linestyle=':', linewidth=0.7, alpha=0.5, color='#cccccc', zorder=0)
...
class MacroRatioChartGenerator(ChartGenerator):
    ...
    def generate_html(self, protein_pct: float, carbs_pct: float, fat_pct: float) -> str:
        ...
        html.append(f"  <div class='progress-protein' style='width: {protein_pct_rounded}%'>" + 
                   (f"<span class='macro-text'>Protein {protein_pct_rounded}%</span>" if protein_pct >= 15 else 
                    f"<span class='macro-text'>{protein_pct_rounded}%</span>" if protein_pct >= 8 else "") + 
                   "</div>" +
                   f"<div class='progress-carbs' style='width: {carbs_pct_rounded}%'>" + 
                   (f"<span class='macro-text'>Carbs {carbs_pct_rounded}%</span>" if carbs_pct >= 15 else 
                    f"<span class='macro-text'>{carbs_pct_rounded}%</span>" if carbs_pct >= 8 else "") + 
                   "</div>" +
                   f"<div class='progress-fat' style='width: {fat_pct_rounded}%'>" + 
                   (f"<span class='macro-text'>Fat {fat_pct_rounded}%</span>" if fat_pct >= 15 else 
                    f"<span class='macro-text'>{fat_pct_rounded}%</span>" if fat_pct >= 8 else "") + 
                   "</div>")
