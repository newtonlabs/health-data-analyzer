...
from src.utils.date_utils import DateUtils, DateStatus
...
class HealthDataProcessor:
    ...
    def _process_oura_resilience(self, raw_data: Dict[str, Any], start_date: datetime, end_date: datetime) -> pd.DataFrame:
        ...
        for item in raw_data['data']:
            ...
            try:
                ...
                if day_date.date() < start_date.date() or day_date.date() > end_date.date():
                    continue
                ...
                resilience_score = AnalyzerConfig.RESILIENCE_LEVEL_SCORES.get(resilience_level.lower(), 0)
                ...
                self.logger.log_skipped_date(None, f"Error processing resilience data: {str(e)}")
                continue
    ...
    def process_whoop_data(self, raw_data: Dict[str, Any]) -> Dict[str, pd.DataFrame]:
        ...
        workouts_df = pd.DataFrame(columns=['date', 'sport_id', 'sport', 'duration', 'strain'])
        recovery_df = pd.DataFrame(columns=['date', 'recovery_score', 'hrv_rmssd', 'resting_hr'])
        ...
        if 'sleep' in raw_data and 'records' in raw_data['sleep'] and recovery_data:
            ...
            for sleep in raw_data['sleep']['records']:
                ...
                created = datetime.fromisoformat(sleep['created_at'].replace('Z', '+00:00')).replace(tzinfo=None)
                ...
                total_sleep_milli = stage_summary.get('total_in_bed_time_milli', 0) - stage_summary.get('total_awake_time_milli', 0)
                
                sleep_by_date[sleep_date.strftime('%Y-%m-%d')] = {
                    'sleep_need': total_need_milli / (3600 * 1000),  # Convert milliseconds to hours
                    'sleep_actual': total_sleep_milli / (3600 * 1000)  # Convert milliseconds to hours
                }
    ...
    def _transform_workout(self, workout: Dict[str, Any]) -> Dict[str, Any]:
        ...
        start_time = (start_time_utc + timedelta(seconds=offset_seconds)).replace(tzinfo=None)
        end_time = (end_time_utc + timedelta(seconds=offset_seconds)).replace(tzinfo=None)
    ...
    def _transform_recovery(self, recovery: Dict[str, Any]) -> Dict[str, Any]:
        ...
        created_utc = datetime.fromisoformat(recovery['created_at'].replace('Z', '+00:00'))
