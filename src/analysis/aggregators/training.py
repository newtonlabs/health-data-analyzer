...
class TrainingAggregator(BaseAggregator):
    ...
    def _format_workout_data(self, workout_df: pd.DataFrame) -> dict:
        ...
        self.logger.logger.debug(
            f"_format_workout_data - columns: {workout_df.columns}"
        )
        self.logger.logger.debug(
            f"_format_workout_data - sample data: {workout_df.head(1).to_dict('records')}"
        )
        ...
        for _, row in workout_df.iterrows():
            ...
            if "duration" in row:
                ...
                self.logger.logger.debug(
                    f"_format_workout_data - workout on {date}: sport={sport}, duration={duration}, type={type(duration)}"
                )
            else:
                duration = 0
                self.logger.logger.debug(
                    f"_format_workout_data - workout on {date}: sport={sport}, duration missing, using default 0"
                )
