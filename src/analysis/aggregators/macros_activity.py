    ...
    def weekly_macros_and_activity(
        ...
        for date in df["date"]:
            if date in nutrition_by_date:
                df.loc[df["date"] == date, "calories"] = nutrition_by_date[date][
                    "calories"
                ]
                df.loc[df["date"] == date, "protein"] = nutrition_by_date[date][
                    "protein"
                ]
                df.loc[df["date"] == date, "carbs"] = nutrition_by_date[date]["carbs"]
                df.loc[df["date"] == date, "fat"] = nutrition_by_date[date]["fat"]
                df.loc[df["date"] == date, "alcohol"] = nutrition_by_date[date][
                    "alcohol"
                ]
    ...
    def _add_activity_data(
        ...
        if "duration" in workout_df.columns:
            self.logger.logger.debug(
                f"Workout durations: {workout_df['duration'].tolist()}"
            )
        else:
        ...
        for _, row in workout_df.iterrows():
            ...
            self.logger.logger.debug(
                f"Processing workout: date={date_key}, sport={sport}, strain={strain}, duration={duration}"
            )
