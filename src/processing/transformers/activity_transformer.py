...
from src.models.raw_data import ActivityRecord
...
class ActivityTransformer(RecordListTransformer[ActivityRecord]):
    ...
    def transform_record(self, record: ActivityRecord) -> Optional[ActivityRecord]:
        ...
        if final_date is None:
            # If no date available, we can't process this record
            self.logger.warning("No date available for activity record")
            return None
